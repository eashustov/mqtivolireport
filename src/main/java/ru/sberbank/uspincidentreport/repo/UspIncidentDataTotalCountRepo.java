...
//            nativeQuery = true)
        @Query(value = "select p.HPC_ASSIGNMENT as Assignment, COUNT (p.NUMBER) AS countInc from probsummarym1 p where p.OPEN_TIME BETWEEN TO_CHAR(:#{#startDate} '00:00:00', 'DD.MM.RRRR HH24:MI:SS') AND TO_CHAR(" + "'" + :endDate + "23:59:59', 'DD.MM.RRRR HH24:MI:SS') GROUP BY Assignment ORDER BY countInc DESC", nativeQuery = true)
