...
@Repository
public interface UspIncidentCountRepo extends CrudRepository<UspIncidentData, String> {
    ...
    @Query(value = "SELECT\n" +
            ...
            "\t'SberInfra УСП Интеграционные платформы (Гоголев К.Ю.) (00019273)')) \n" +
            "WHERE\n" +
            "\tOPENED_BY = 'int_zabbix_si' AND OPEN_TIME BETWEEN TO_TIMESTAMP('01.04.2022 8:23:16', 'DD.MM.RRRR HH24:MI:SS') AND TO_TIMESTAMP('17.05.2022 8:23:16', 'DD.MM.RRRR HH24:MI:SS')\n" +
            "\tGROUP BY \"HPC_ASSIGNMENT\" ORDER BY \"COUNT_INC\" DESC",
            nativeQuery = true)
    List<UspIncidentData> findIncCount();
